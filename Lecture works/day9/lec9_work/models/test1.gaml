...
//	int neighbours_distance <- 50 min: 1 max: 1000;
	float neighbours_distance <- 50.0 min: 1.0 max: 1000.0;
...
experiment explo type: batch 
	repeat: 8 keep_seed: true 
//	until: (cycle > 100)
	until: (length(people where !each.is_happy) = 0) or (cycle > 100)
{	
	
//	parameter var: numberPeople init: 2000 min: 1000 max: 10000 step: 100;
	parameter var: number_of_people among: [2000];

//	parameter var: neighbors_distance init: 5 #m;
	parameter var: neighbours_distance among: [5.0 #m];
	
	parameter var: percent_similar_wanted
		min: 0.3 max: 1.0 step: 0.1;
	

	permanent {
		display dPermanent {
			chart "Epected chart" type: series 
									x_serie_labels: (percent_similar_wanted) 
									x_label: "rate_similar_wanted" 
			{
				data "average_end_cycle" value: simulations mean_of(each.cycle) 
					y_err_values: [
						(simulations mean_of(each.cycle)) - mean_deviation(simulations collect(each.cycle)) ,
						(simulations mean_of(each.cycle)) + mean_deviation(simulations collect(each.cycle))
					]
					thickness: 3;
				data "Max cycle" value: simulations max_of(each.cycle);
				data "Min cycle" value: simulations min_of(each.cycle);
			}
	...
	init {
		save ["NBPeople", "Tolerance", "cycle", "happyPeople"]
			to: "resultVar.csv" format: "csv" rewrite: true header: false;
	}	
	
	reflex saveCSV {
		ask simulations {
			save [self.number_of_people, self.percent_similar_wanted, self.cycle, self.people count(each.is_happy)]
				to: "resultVar.csv" format: "csv" rewrite: false;
		}
	}
	
//	exploration exhautive;
