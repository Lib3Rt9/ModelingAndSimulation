...
global {
    ...
    float system_perturbation <- 0.0              			// Current perturbation level
    			update: perturbation_active ? perturbation_strength : 0.0;
    ...
    float current_perturbation_level <- 0.0;                // Track current perturbation level
    
    float perturbation_recovery_rate <- 0.002; 	 			// Rate of recovery from perturbation
	float group_resilience_bonus <- 0.2;        			// Groups are more resilient to perturbations
    ...
    float avg_group_collection_rate <- 0.0 
	    update: empty(Collector where each.in_group) ? 0.0 : 
	...
	float calculate_system_resilience {
	    if (perturbation_active) {
	        ...
	        return max(0.0, min(1.0, (group_performance / max(0.001, solo_performance)))) 
	            with_precision 2;
	    } else {
	        // During normal operation, measure recovery potential
	        return (group_stability_index * 0.7 + system_stability * 0.3) with_precision 2;
	    }
	}
    
    /////////////////////////////////////////////////////////////////////////////////////////////
    //////
    int cleanup_interval <- 200;  							// Cleanup every 200 cycles
...
//            write "Created wood at: (" + spawn_x + ", " + spawn_y + ")";
    ...
    reflex apply_system_perturbation when: apply_perturbation 
    									and every(perturbation_interval) 
    {
        write "Applying system perturbation...";  
        
	    current_perturbation_level <- perturbation_strength;
        ...
        ask Collector {
        	// Store old capacity before reduction
        	int old_capacity <- max_carrying_value;
        	
            // Temporarily reduce effectiveness
            max_carrying_value <- max(1, int(max_carrying_value * (1.0 - system_perturbation)));
	        speed <- speed * (1.0 - system_perturbation);
	        steal_chance <- steal_chance * (1.0 - system_perturbation);
	        
	        // Check if currently carrying more than new capacity
	        if (current_carried_value > max_carrying_value) {
	            write "Collector " + self + " needs to drop wood pieces due to perturbation";
	            
	            // Keep dropping pieces until within new capacity
	            loop while: (current_carried_value > max_carrying_value) and !empty(carried_wood) {
	                // Get last wood piece
	                Driftwood piece_to_drop <- last(carried_wood);
	                
	                // Drop the piece
	                piece_to_drop.is_collected <- false;
	                piece_to_drop.in_pile <- false;
	                piece_to_drop.location <- location;  // Drop at current location
	                
	                // Update collector's status
	                carried_wood <- carried_wood - piece_to_drop;
	                current_carried_value <- current_carried_value - piece_to_drop.value;
	                
	                // Create warning message for dropped wood
	        ...
	        // Group-specific perturbations
	        if (in_group) {
	            group_formation_chance <- group_formation_chance * (1.0 - system_perturbation);
	            cooperation_bonus <- cooperation_bonus * (1.0 - system_perturbation);
        	}
        }
        ...
        create WarningMessage {
	        ...
	        text <- "System Perturbation " + system_perturbation + " Applied!";
	        duration <- 100;
        }
    }
    
    // Recovery from pertubation
    reflex recover_from_perturbation when: perturbation_active 
//							and (cycle - perturbation_start_time >= perturbation_recovery_time) 
	{
        write "Recovering from perturbation...";
        
	    // Gradual recovery instead of instant
	    current_perturbation_level <- current_perturbation_level - perturbation_recovery_rate;
	    
	    if (current_perturbation_level <= 0.0) {
	        write "System fully recovered from perturbation";
	        current_perturbation_level <- 0.0;
	        perturbation_active <- false;
	        
	        // Restore collector values with group bonus
	        ask Collector {
	            max_carrying_value <- 10;
	            speed <- rnd(0.0, 8.0);
	            ...
	            if (in_group) {
	                ...
	                steal_chance <- steal_chance * (1 - group_resilience_bonus);
	            }
	        }
	        
	        create WarningMessage {
	            location <- {world.shape.width/2, world.shape.height/2};
	            text <- "System Recovered from Perturbation";
	            duration <- 50;
	        }
	    }
    }
...
species Driftwood {
    ...
    init {
        ...
        float size_factor <- size_category = "small" ? 1.0 : (size_category = "medium" ? 1.5 : 2.0);
		width <- 0.8 * size_factor;
		height <- 0.4 * size_factor;
		value <- size_category = "small" ? 1 : (size_category = "medium" ? 2 : 5);
    }
...
species Collector skills: [moving] {
	...
	reflex return_to_pile when: (!empty(carried_wood) and (
	    ...
	    if (location distance_to pile_location < 1.0) {
	        ...
	        // Update personal stability based on successful pile creation
            personal_stability <- personal_stability + 0.1;
            if (personal_stability > 1.0) { personal_stability <- 1.0; }
    ...
    reflex consider_forming_group 
    			...
    			and every(50 #cycles) 
    {
        // Don't form groups during perturbations
		if (perturbation_active) { return; }
        
        list<Collector> potential_members <- Collector at_distance view_distance 
        														where (!each.in_group);
        // Calculate optimal group size based on current conditions
	    int optimal_size <- int(min_group_size + 
	        (max_group_size - min_group_size) * 
	        (1.0 - system_perturbation)  // Smaller groups during perturbation recovery
	    );
        
        if (length(potential_members) >= optimal_size - 1) {
            // Initialize group with optimal size
            ...
            // Add random members up to optimal size
            loop times: optimal_size - 1 {
	            Collector new_member <- potential_members with_min_of 
            ...
            write "Group " + group_id + " formed with " + length(new_group) + " members (Optimal size: " + optimal_size + ")." ;
        }
    }
    ...
    // Performance impact from perturbations
    reflex update_performance when: perturbation_active {
        personal_stability <- personal_stability - (system_perturbation * 0.1);
        if (personal_stability < 0.0) { personal_stability <- 0.0; }
        
        // Group members recover stability faster
        if (in_group) {
            personal_stability <- personal_stability + 0.05;
            if (personal_stability > 1.0) { personal_stability <- 1.0; }
        }
    }
        ...
        loop collector over: suspicious_collectors {
            ...
            loop pile over: nearby_piles {
                if (pile.owner != collector and 
                    ...
                    write "- " + self + " detected suspicious behavior near pile " + pile.location+ "!";
                    pursuit_target <- collector;
                    is_pursuing <- true;
                    break;
                }
    ...
    output {
        display main_display {
            ...
            graphics "Info Display" {
                ...
                draw "Tide Level: " + (round(tide_level * 100) / 100) at: {2, 10} color: #orange font: font("Default", 10, #bold);
                
                // Legend for collector information
                draw "Legend:" at: {2, 20} color: #orange font: font("Default", 10, #bold);
                draw "V: Speed (km/h)" at: {2, 25} color: #orange font: font("Default", 10, #bold);
                draw "C: Current/Max Capacity" at: {2, 30} color: #orange font: font("Default", 10, #bold);
                draw "S: Steal Chance %" at: {2, 35} color: #orange font: font("Default", 10, #bold);
                draw "L/M: Leader/Member" at: {2, 40} color: #orange font: font("Default", 10, #bold);
                draw "D: Water Depth" at: {2, 45} color: #orange font: font("Default", 10, #bold);
            }
        }
        
        // Essential monitors
        monitor "Cycle" value: cycle;
        monitor "Time" value: (int(day_time) < 10 ? "0" : "") + string(int(day_time)) + ":" + 
                            (int((day_time - int(day_time)) * 60) < 10 ? "0" : "") + 
                            string(int((day_time - int(day_time)) * 60));
        
        // Combined system metrics
        monitor "System Overview" value: "Stability: " + (system_stability with_precision 2) + 
                                      " | Resilience: " + (system_resilience with_precision 2);
        
        // Group metrics
        monitor "Group Statistics" value: "Groups: " + length(Collector where each.is_group_leader) + 
                                       " | Avg Size: " + (empty(Collector where each.in_group) ? 0 : 
                                       mean(Collector where each.in_group collect length(each.group_members)) 
                                       with_precision 1);
                                
        // Group performance
        monitor "Group Performance" value: "Efficiency: " + 
            (avg_group_collection_rate with_precision 2) + 
            " | Stability: " + (group_stability_index with_precision 2);
        
        // Resource status
        monitor "Resources" value: "Wood: " + length(Driftwood) + 
                                 " | Piles: " + length(WoodPile) + 
                                 " | Highest Value: " + (empty(WoodPile) ? 0 : max(WoodPile collect each.pile_value));
        
        // Security metrics
        monitor "Security" value: "Catches: " + total_catches + 
                                " | Active Pursuits: " + active_pursuits;
        
        // Combined performance charts
        display "System Performance" {
            chart "Performance Metrics" type: series {
                data "Group Efficiency" value: avg_group_collection_rate * 100 color: #blue;
                data "System Stability" value: system_stability * 100 color: #green;
                data "System Resilience" value: system_resilience * 100 color: #yellow;
                data "Perturbation Level" value: perturbation_active ? current_perturbation_level * 100 : 0 
                    color: #red;
            }
        }
        ...
        display "Resource Distribution" {
            chart "Wood Distribution" type: pie {
                data "Group Collectors" value: sum((Collector where each.in_group) 
                    collect each.current_carried_value) color: #green;
                data "Solo Collectors" value: sum((Collector where !each.in_group) 
                    collect each.current_carried_value) color: #blue;
                data "Uncollected" value: sum(Driftwood where (!each.is_collected) 
                    collect each.value) color: #gray;
            }
        }
    }
