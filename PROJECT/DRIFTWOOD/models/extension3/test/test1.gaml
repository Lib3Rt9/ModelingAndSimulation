...
global {
    ...
    float avg_group_collection_rate <- 0.0 
        		update: empty(group_efficiency.values) ? 0.0 : 
                mean(group_efficiency.values);
...
//            write "Created wood at: (" + spawn_x + ", " + spawn_y + ")";
    ...
    float calculate_solo_vs_group_efficiency {
        ...
        float solo_efficiency <- empty(solo_collectors) ? 0.0 : 
            mean(solo_collectors collect (each.successful_steals));
        float grouped_collector_efficiency <- empty(grouped_collectors) ? 0.0 : 
            mean(grouped_collectors collect (each.successful_steals));
            
        return empty(grouped_collectors) ? 0.0 : 
            (grouped_collector_efficiency - solo_efficiency) / max(1.0, grouped_collector_efficiency);
    }
    ...
    output {
	   ...
	   // Wood pile
		...
		display "Group Performance Analysis" refresh: every(5 #cycles) {
            chart "Group vs Solo Performance" type: series style: line background: rgb(255,255,255) size: {1.0, 0.5} position: {0, 0} {
                data "Avg Group Collection" value: avg_group_collection_rate color: #blue;
                data "System Stability" value: system_stability color: #green;
                data "Perturbation Level" value: system_perturbation * 2 color: #red; // Scaled for visibility
            }
            
            chart "Group Distribution" type: pie background: rgb(255,255,255) size: {1.0, 0.5} position: {0, 0.5} {
                data "Solo Collectors" value: length(Collector where (!each.in_group)) color: #gray;
                
                // Add data for each existing group
                ask Collector where (each.is_group_leader) {
                    data "Group " + group_id value: length(group_members) 
