...
//            write "Created wood at: (" + spawn_x + ", " + spawn_y + ")";
    ...
    float calculate_solo_vs_group_efficiency {
        ...
        // Calculate efficiencies using wood_collected_count
					...
					(cycle - perturbation_start_time >= perturbation_recovery_time) 
	{
        ...
        ask Collector {
	        ...
	        // Restore group-related values
	        if (in_group) {
	            group_formation_chance <- 0.3;  			// Reset to default
	            cooperation_bonus <- 0.2;       			// Reset to default
	        }
        }
    ...
    output {
	   ...
	   // Wood pile
		...
		display "Group Performance Analysis" refresh: every(5 #cycles) {
		    ...
		    {
		        data "Avg Group Collection" value: mean(Collector where (each.in_group) 
		        									collect each.wood_collected_count
		        								) color: #blue;
		    ...
		    {
		        data "Solo Collectors" value: length(Collector where (!each.in_group)) 
		        							color: #gray;
