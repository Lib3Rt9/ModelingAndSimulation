...
global {
    ...
    int current_group_id <- 0 
    			update: empty(Collector where each.is_group_leader) ? 0 : 
                              max(Collector where each.is_group_leader 
    ...
    // Group Analysis
    map<int, float> group_efficiency <- [];         		// Track efficiency per group
    map<int, int> group_total_collected <- [];      		// Track total wood collected per group
    ...
    float avg_group_collection_rate <- 0.0 
    			update: 
	    			length(Collector where each.in_group) > 0 ? 
	    			mean(Collector 
	    				where each.in_group 
	    				collect each.collection_efficiency
	    			) : 0.0;
	
	float solo_vs_group_efficiency <- 0.0 
				update: calculate_solo_vs_group_efficiency();
...
//            write "Created wood at: (" + spawn_x + ", " + spawn_y + ")";
    ...
    // Calculate relative efficiency between solo and grouped collectors
    float calculate_solo_vs_group_efficiency {
	    ...
	    if (empty(grouped_collectors) or empty(solo_collectors)) { return 0.0; }
	    
	    float solo_eff <- mean(solo_collectors collect each.collection_efficiency);
	    float grouped_eff <- mean(grouped_collectors collect each.collection_efficiency);
	    
	    return (grouped_eff - solo_eff) / max(1.0, grouped_eff);
	}
    ...
    reflex apply_system_perturbation when: apply_perturbation and every(perturbation_interval) {
        ...
        ask Collector {
            ...
            max_carrying_value <- int(max_carrying_value * (1.0 - system_perturbation));
...
species Collector skills: [moving] {
	...
	reflex return_to_pile when: (!empty(carried_wood) and (
	    ...
	    if (location distance_to pile_location < 1.0) {
	        ...
	        // Update group efficiency if in group
	        if (in_group and group_id >= 0) {
	            ask world {
	                // Update group collection metrics
	                ...
	                // Calculate and update group efficiency
    ...
    aspect default {
        ...
        // Group leader crown
        if (is_group_leader) {
            draw triangle(1.5) color: #gold at: {location.x, location.y - 1.5} rotate: 180;
        }
    ...
    output {
        display main_display {
	        ...
	        graphics "Info Display" {
			    ...
			    // Time
			    draw "Time: " + hours + ":" + minutes at: {2, 5} color: #black font: font("Default", 10, #bold);
			    
			    // Notes
			    draw "Speed: V km/h (1) " at: {2, 15} color: #black font: font("Default", 10, #bold);
			    draw "Capacity: C / max 10 (2)" at: {2, 20} color: #black font: font("Default", 10, #bold);
			    draw "Steal ratio: S % (3)" at: {2, 25} color: #black font: font("Default", 10, #bold);
			    draw "Group (id): L - M (4)" at: {2, 30} color: #black font: font("Default", 10, #bold);
			    draw "Depth: D meter (5)" at: {2, 35} color: #black font: font("Default", 10, #bold);

			    // Tide
			    draw "Tide Level: " + (round(tide_level * 100) / 100) at: {2, 80} color: #black font: font("Default", 10, #bold);
			    ...
			    draw "Tide Trend: " + tide_status + " " + tide_direction at: {2, 85} color: #black font: font("Default", 10, #bold);
			    ...
			    draw "Tide Status: " + tide_status at: {2, 90} color: #black font: font("Default", 10, #bold);
				        ...
				        if (grid_loc.x >= 0 and grid_loc.x < environment_width and 
				            ...
				            if (cell != nil) {
				                draw "Height: " + (round(cell.height * 100) / 100) + "m" 
				                    at: {2, 95} color: #black font: font("Default", 10, #bold);
				            }
				        }
				    }
	        }
	   ...
	   // Wood pile
		...
		display "Group Performance Analysis" refresh: every(5 #cycles) {
	        chart "Group vs Solo Performance" type: series style: line {
	            // Simplified data collection for better visualization
	            data "Avg Group Collection" value: length(Collector where each.in_group) > 0 ? 
	                mean(Collector where each.in_group collect each.collection_efficiency) : 0.0 
	                color: #blue;
	        ...
	        chart "Group Distribution" type: pie {
	            ...
	            // Add data for each active group
	            loop gid over: remove_duplicates(Collector where each.in_group collect each.group_id) {
	                data "Group " + gid value: length(Collector where (each.group_id = gid)) 
	                    color: rgb(rnd(100,255), rnd(100,255), rnd(100,255));
	            }
	        }
	    }
        ...
        display "Performance Analysis" refresh: every(5 #cycles) {
            chart "Detailed Metrics" type: series style: line {
	            data "Group vs Solo Efficiency" value: solo_vs_group_efficiency color: #blue;
	            data "System Stability" value: system_stability color: #green;
	            data "Active Groups" value: length(Collector where each.is_group_leader) color: #orange;
	            data "Perturbation Active" value: perturbation_active ? 1.0 : 0.0 color: #red;
	        }
	        
//	        chart "Detailed Metrics" type: series style: line background: rgb(255,255,255) {
//                data "Group Efficiency" value: solo_vs_group_efficiency color: #blue;
//                data "System Stability" value: system_stability color: #green;
...
//            }
		...
		monitor "Average Collection Rate" value: avg_group_collection_rate;
		monitor "Solo vs Group Efficiency" value: solo_vs_group_efficiency;
		monitor "System Perturbation Level" value: system_perturbation;
		monitor "Most Efficient Group" value: empty(group_efficiency.pairs) ? -1 : 
		    (group_efficiency.pairs with_max_of each.value).key;
