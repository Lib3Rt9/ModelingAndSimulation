...
global {
    ...
    float wood_spawn_rate <- 0.05;      					// Probability of spawning wood per step


	// Collector parameters
	int num_collectors <- 10;							// Number of collector(s)
	float movement_time_factor <- 1.0;  				// Base factor for movement scaling
...
//            write "Created wood at: (" + spawn_x + ", " + spawn_y + ")";
    ...
    reflex check_end_condition {
        ...
        list<WoodPile> winning_piles <- WoodPile where (each.pile_value >= 20);
        
        if (!empty(winning_piles) and !simulation_ended) {
...
species Collector skills: [moving] {
    ...
    float speed <- rnd(0.0, 8.0);  					// Speed random at from 0.0 to 8.0 km/h
    float rnd_speed <- rnd(1.0, 10.0);
    float acceleration <- 0.1;                   		// Gradual speed change
    float max_speed_change <- 2.0;              		// Maximum speed change allowed
    ...
    reflex update_speed {
        ...
        if (!empty(carried_wood)) {
            ...
            reduction_factor <- max(0.1, reduction_factor);	// Min speed = 10% original speed
    ...
    reflex move_to_wood when: targeted_wood != nil {
        ...
        if (location distance_to targeted_wood < 1.0) {
            ...
            // Look for next wood if capacity allows
            list<Driftwood> available_wood <- Driftwood 
                where (
                    !each.is_collected 
                and 
            ...
            targeted_wood <- nil;  // Wood was taken, reset target
        }
...
species WoodPile {
    ...
    reflex update_pile_value {
        list<Driftwood> woods_in_pile <- Driftwood 
        	where (each.location = location 
        		and 
        			each.in_pile
        	);
    ...
    aspect default {
        if (has_marker) {
            ...
            // Show pile value
            draw string(pile_value) color: #black size: 8 at: {location.x, location.y - 1};
        }
    }
...
experiment Driftwood_Simulation type: gui {
    ...
    parameter "Wood Spawn Rate" var: wood_spawn_rate min: 0.01 max: 0.1;
